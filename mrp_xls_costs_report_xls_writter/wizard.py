...
class agged_xls_export(osv.osv_memory):
    ...
    def export_xls_file(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=None):
            ...
            worksheet.write('F1', 'Fecha Inicio', format_red)
            worksheet.write('G1', 'Fecha Fin', format_red)

            worksheet.write('A2', model_br.bom_id.name)
            worksheet.write('B2', model_br.product_id.name)
            worksheet.write('C2', model_br.uom_id.name)
            worksheet.write('D2', model_br.product_qty)
            worksheet.write('E2', model_br.product_amount_total)
            worksheet.write('F2', model_br.date_start)
            worksheet.write('G2', model_br.date_end)
            ...
            cl = len(model_br.consumed_lines)+10
            # print "############# CL", cl
            i = 6
            for line in model_br.consumed_lines:
                worksheet.write('A'+str(i), line.name)
                worksheet.write('B'+str(i), line.product_id.name)
                worksheet.write('C'+str(i), line.uom_id.name)
                worksheet.write('D'+str(i), line.product_qty)
                worksheet.write('E'+str(i), line.product_cost)
                worksheet.write('F'+str(i), line.product_total)
                i+=1
            worksheet.write('E'+str(i), 'Total', format_black)
            worksheet.write('F'+str(i), model_br.product_amount_lines if model_br.product_amount_lines else 0.0)
            ...
            for cost in model_br.cost_lines:
                worksheet.write('A'+str(j), cost.name)
                worksheet.write('B'+str(j), cost.product_id.name)
                worksheet.write('C'+str(j), cost.product_total)
                j+=1
            worksheet.write('B'+str(j), 'Total', format_black)
            worksheet.write('C'+str(j), model_br.product_cost_lines)
            workbook.close()
